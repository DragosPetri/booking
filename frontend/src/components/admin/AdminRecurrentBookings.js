...
const AdminRecurrentBookings = () => {
  ...
  const columns = [
    ...
    { dataField: 'next_date', text: 'Booking Next Date' },
  ];

  const getBookings = () => {
    (async () => {
      const result = await axios.get('http://localhost:9090/api/centres/recurrent');
  ...
  const deleteRecurrentBooking = (id) => {
    (async () => {
    ...
    renderer: (row) => (
      <div className="list-group">
        <span className="list-group-item list-group-item-action list-group-item-danger d-flex justify-content-between">
          <div className="w-100">
            <h5 className="mb-1">Delete recurrent booking</h5>
            <p className="mb-1">This action is permanent.</p>
            <small className="text-muted">This will cancel the user&lsquo;s plan but will not delete the existing bookings issued by this reccurent setting.</small>
          </div>
          <button type="button" onClick={() => { deleteRecurrentBooking(row.id); }} className="btn btn-danger align-self-center">Delete</button>
        </span>
      </div>
    ),
...
export default AdminRecurrentBookings;
